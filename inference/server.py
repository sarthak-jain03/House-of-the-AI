...
if not FIREWORKS_API_KEY:
    raise RuntimeError("FIREWORKS_API_KEY missing in environment")

# USE VALID MODELS
MODELS = {
    "poet": "accounts/fireworks/models/llama4-maverick-instruct-basic",
    "coder": "accounts/fireworks/models/llama4-maverick-instruct-basic",
    "story": "accounts/fireworks/models/llama4-maverick-instruct-basic",
    "analyzer": "accounts/fireworks/models/llama4-maverick-instruct-basic"
}
...
def fireworks_chat(model, messages, temperature=0.5, max_tokens=1000):
    headers = {
...
def compute_advanced_eda(df):
    ...
    for col in num_cols:
        ...
        if abs(skew) > 1:
            ...
            fix = "Try normalization"

        else:
            fix = "Looks approximately normal"
    ...
    if len(cat_cols) > 0:
        rec.append("Use encoding for categorical columns.")

    if len(num_cols) > 0:
        rec.append("Scale numerical columns.")
...
def story_weaver():
    ...
    if action == "write_script":
        instruction = (
            f"Convert to screenplay:\n\n{story}\n\n{prompt}"
        )

    elif action == "add_character":
        instruction = (
            f"Add a new character:\n\n{story}\n\n{prompt}"
        )

    elif action == "plot_outline":
        instruction = (
            f"Create a plot outline:\n\n{story}\n\n{prompt}"
        )

    elif action == "alt_directions":
        instruction = (
            f"Generate alternate paths:\n\n{story}\n\n{prompt}"
        )
